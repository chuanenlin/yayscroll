...
- Start directly with the content (no numbering prefix)`
        ...
        // Instead of creating misleading content, just log the error and continue
        // The client will get existing content, which is better than fake content
        if (generationError instanceof Error) {
          console.error('🚨 [API] Error details:', generationError.message)
        }
        
        // Don't create fallback content - let the application handle the lack of new content gracefully
        console.log('🔄 [API] Continuing with existing content after generation failure')
      } finally {
        // Always clear generation lock when we're done
        console.log(`🔓 [API] Releasing generation lock`)
        rateLimit.isGenerating = false
        delete rateLimit.generationStartTime
      }
    }
